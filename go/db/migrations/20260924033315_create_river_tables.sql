...
-- バックグラウンドジョブキューのRiverが使うテーブルを作る。
--
-- RiverはスキーマのマイグレーションをGoのライブラリとして持つが、Cutreのスキーマの正本はdbmateとdb/schema.sqlに置く。
-- Riverのマイグレーションをアプリケーションの起動時に走らせると、スキーマを変える経路が2つになり、
-- db/schema.sqlとデータベースの一致をdbmateだけで保証できなくなるため。
--
-- 本文はRiver v0.47.0のCLIが出力したSQLをそのまま貼っている (version 1から7まで)。
-- 手で書き直さないのは、同じコマンドを実行し直せば差分で内容を確かめられるようにするため。
--
--   go run github.com/riverqueue/river/cmd/river@v0.47.0 migrate-get --line main --all --up
--   go run github.com/riverqueue/river/cmd/river@v0.47.0 migrate-get --line main --all --down
--
-- version 1が作る追跡テーブルriver_migrationも残し、末尾で適用済みのversionを記録する。
-- Riverのライブラリ (rivermigrate) からスキーマがどこまで進んでいるかを読めるようにするため。
--
-- Riverを上げて新しいversion Nが増えたときは、次の手順で追随する。
--
-- 1. 上のコマンドで --all の代わりに --version N を指定し、up / downのSQLを出力する
-- 2. `make db-new` で作ったマイグレーションに貼り、upの末尾で ('main', N) をriver_migrationに記録する (downでは削除する)
-- 3. internal/worker の appliedRiverMigrationVersion をNに上げる
--
-- 以降の本文のコメントはRiverのCLIが出力したもの。
...
-- 適用済みのversionを記録する。Riverのマイグレーターを使わないため、記録は自分で行う。
