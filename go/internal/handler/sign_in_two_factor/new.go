...
// New GET /sign_in/two_factor (日本語版) と GET /en/sign_in/two_factor (英語版) - 認証アプリのコードの入力画面を描画する。
//
...
// render は認証アプリのコードの入力画面を指定したステータスで描画する。
...
func (h *Handler) render(w http.ResponseWriter, r *http.Request, status int, data page.NewPageData) {
	...
	meta := viewmodel.DefaultPageMeta(ctx, h.cfg, templates.SignInTwoFactorPath)
	...
	if err := layouts.Default(layouts.DefaultLayoutData{Meta: meta}, page.New(data)).Render(ctx, w); err != nil {
		...
		slog.ErrorContext(ctx, "認証アプリのコードの入力画面の描画に失敗しました", "error", err)
	}
}
