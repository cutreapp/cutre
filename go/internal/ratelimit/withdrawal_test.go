...
// TestLimiter_CheckWithdrawal は、同じユーザーの退会のパスワードの確認を5回までだけ受け付け、6回目で数え直しの時刻を返し、
// ほかのユーザーの回数とは分けて数えることを検証する。
func TestLimiter_CheckWithdrawal(t *testing.T) {
	t.Parallel()
	...
	for i := range 5 {
		resetAt, err := limiter.CheckWithdrawal(ctx, userID)
		if err != nil {
	...
	otherResetAt, err := limiter.CheckWithdrawal(ctx, "user-"+testutil.UniqueAtname())
